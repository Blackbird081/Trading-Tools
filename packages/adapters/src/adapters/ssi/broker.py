...
class SSIBrokerClient:
    ...
    async def _cancel_order_impl(self, order_id: str) -> Order:
        token = await self._auth.get_access_token()
        async def _call() -> dict[str, Any]:
            r = await self._http.post(SSI_CANCEL_URL, json={"account": self._account_no, "orderID": order_id, "requestID": f"cancel-{order_id}", "market": "VN", "instrumentID": "", "buySell": "B", "orderType": "LO", "price": "0", "quantity": 0}, headers={"Authorization": f"Bearer {token}"})
            r.raise_for_status()
            return r.json()  # type: ignore[no-any-return]
        await retry_async(_call, config=_RETRY_CONFIG, operation_name="ssi.cancel_order")
        now = datetime.now(UTC)
        return Order(order_id=order_id, symbol=Symbol(""), side=OrderSide.BUY, order_type=OrderType.LO, quantity=Quantity(0), price=Price(Decimal("0")), ceiling_price=Price(Decimal("0")), floor_price=Price(Decimal("0")), status=OrderStatus.CANCELLED, filled_quantity=Quantity(0), avg_fill_price=Price(Decimal("0")), broker_order_id=order_id, rejection_reason=None, idempotency_key=f"cancel-{order_id}", created_at=now, updated_at=now)
    ...
    def _parse_order(self, data: dict[str, Any]) -> Order:
        ...
        return Order(
            order_id=str(data.get("requestID", "")), symbol=Symbol(str(data.get("instrumentID", ""))),
            side=side, order_type=OrderType(str(data.get("orderType", "LO"))),
            ...
            status=status, filled_quantity=Quantity(int(data.get("filledQty", 0))),
            avg_fill_price=Price(Decimal(str(data.get("avgPrice", "0")))),
            broker_order_id=str(data.get("orderID", "")), rejection_reason=data.get("rejectReason"),
            idempotency_key=str(data.get("requestID", "")), created_at=now, updated_at=now,
        )
