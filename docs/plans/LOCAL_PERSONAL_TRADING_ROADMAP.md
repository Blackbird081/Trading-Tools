...
| Hardening-A4 Frontend Coverage Expansion | `PARTIAL (in progress)` | Expanded DataLoader integration pack (`frontend/__tests__/integration/data-loader.test.tsx`) and fixed stale preset/year race in loader stream (`frontend/app/(dashboard)/_components/data-loader.tsx`); latest release-validation critical frontend snapshot: lines/statements `96.6%`. | Expand risk-based suites for orders/screener/market-board and enforce >=90% critical-flow target in release gate policy. |
| REL-01 Release Quality Gate Uplift (financial-safe) | `DONE (gated)` | `scripts/pre-live-api-gate.ps1` now supports configurable backend threshold and risk-based frontend coverage scope; `scripts/release-validation.ps1` enforces backend + frontend critical gates; latest run passed with backend critical `97.51%` and frontend critical lines/statements `96.6%`. | Keep >=90% critical-flow threshold stable across subsequent release candidates. |
...
- `Hardening-A4` is now in progress: dashboard loader critical-flow coverage was expanded and validated at `96.6%` (critical snapshot scope).
- Next focus: widen Hardening-A4 from dashboard loader to orders/screener/market-board coverage packs for sustained >=90% critical-flow gate confidence.
