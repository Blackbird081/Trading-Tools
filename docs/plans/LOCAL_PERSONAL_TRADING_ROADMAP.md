...
| Phase 3 - OMS and Broker Execution (Dry-Run First) | `DONE` | Added backend OMS REST (`packages/interface/src/interface/rest/orders.py`), idempotency async wiring (`trading_store.py`, `DuckDBIdempotencyStore.create()`), frontend order API integration (`frontend/stores/order-store.ts`, `frontend/app/orders/_components/*`), and SSI live order submission/cancel path with credential-gated runtime + DLQ fallback (`interface/live_broker.py`, `adapters/ssi/broker.py`). | Execute real-account UAT checklist with production credentials before enabling unattended live mode. |
| Phase 4 - Real Portfolio Sync | `DONE` | Implemented `/api/portfolio`, `/positions`, `/pnl`, `/refresh`, `/reconcile` and connected Portfolio UI/store (`frontend/stores/portfolio-store.ts`, `frontend/app/portfolio/*`); live mode now attempts broker snapshot via SSI portfolio client with cache fallback and reconciliation delta output. | Expand broker-side reconciliation depth once SSI account field mapping is confirmed in real account UAT. |
...
| AI-06 Single-Provider Multi-Role Subagent Orchestration | `DONE (gated)` | `FundamentalAgent` now runs contextual subroles (`thesis`, `valuation`, `news_catalyst`, `risk_challenge`) via a single AI engine/API key, enforces schema-locked role outputs (`key_findings`, `recommendation_bias`, `confidence`, `summary`), and applies deterministic arbitration (`risk_veto_then_consensus`); pipeline emits role metadata (`ai_role_outputs`, `ai_subroles`, `ai_final_action`). | Keep schema adherence monitored against real provider responses to avoid prompt drift. |
...
| AI-R5 Provider Parity Calibration | `DONE (baseline)` | Added provider parity evaluator (`tests/evals/provider_parity_calibration.py`) and integrated it into reliability pack + weekly report output (`tests/evals/run_reliability_pack.py`, `scripts/run-weekly-reliability-pack.ps1`) with parity spread/best/worst provider tracking. | Move from fixed dataset baseline to rolling production-like dataset snapshots for calibration policy sign-off. |
